...
describe('GoalsResolver', () => {
            ...
            providers: [
                GoalsResolver,
                {
                    ...
                    useFactory: () => ({
                        ...
                        editGoal: jest.fn((params: {where: Prisma.GoalWhereUniqueInput, data: Prisma.GoalUpdateInput}) => ({
                            id: params.where.id,
                            ...params.data
                        })),
    ...
    it('should return array of goals', async() => {
        let test = await resolver.findGoals();
        expect(test).toEqual([
            {
                id: 1, 
                text: 'First Goal',
    ...
    it('should return one goal with the given id', async() => {
        let test = await resolver.findGoal(1);
        expect(test).toEqual(
            {
                id: 1, 
                text: 'First Goal',
    ...
    it('should update one existing goal with the given input', async() => {
        ...
        let test = await resolver.editGoal(testGoal.id, testGoal.text, testGoal.completed);
